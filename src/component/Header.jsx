...
function Header() {
  const { currentPageHeader } = usePage();
  ...
  return (
    <div id="header" className="header">
      <div className="contents">
        <div className={`main-contents ${open ? "open" : ""}`}>
          <h1>P O L T F L O L I O</h1>
          <p className="title">Web FrontEnd & Publisher</p>
          <p>웹 개발과 퍼블리싱을 같이하는</p>
          <p>차별화된 개발자입니다.</p>
          <p>개인 프로젝트로써 해당 포트폴리오를 제작하였습니다.</p>
          <Link to={pageIndex.current_intro} className="btn-about">
            더 알아보기
          </Link>
        </div>
      </div>
